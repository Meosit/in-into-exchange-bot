Конвертация сумм с поддержкой {currency_count} валют, арифметических и многовалютных выражений.
...
- Курсы на конкретную дату могут быть получены через опцию `at 2022-09-14` либо через использование относительной даты `? -4` (четыре дня назад)
